...
export default function App() {
  ...
  const { timerState, timerConfig, bgTheme = 'pachamama', bgOpacity = 30, competitionLogo, localIp } = ws.state;
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#0f0f14' }}>
      ...
      <header className="glass-card" style={{ margin: '16px 16px 0', padding: '12px 20px', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyItems: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: ws.connected ? '#10b981' : '#ef4444' }} />
            <span style={{ fontSize: 10, fontWeight: 700, color: '#a1a1aa' }}>{ws.connected ? 'ONLINE' : 'OFFLINE'}</span>
          </div>
        </div>
      </header>
      ...
      {showQR && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowQR(false)}>
          <div style={{ background: '#18181b', padding: '32px', borderRadius: '24px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ fontSize: 20, fontWeight: 900, color: '#fff', margin: '0 0 8px' }}>Control Remoto</h2>
            <p style={{ fontSize: 14, color: '#a1a1aa', margin: '0 0 24px' }}>Escanea este código con tu teléfono móvil para controlar el timer.</p>
            <div style={{ background: '#fff', padding: '16px', borderRadius: '16px', display: 'inline-block', marginBottom: '24px' }}>
              <QRCode value={localIp ? `http://${localIp}:3001/remote` : `http://${window.location.hostname}:3001/remote`} size={256} />
            </div>
            <div>
              <p style={{ fontSize: 12, fontFamily: 'monospace', color: '#71717a', margin: '0 0 16px' }}>{localIp ? `http://${localIp}:3001/remote` : `http://${window.location.hostname}:3001/remote`}</p>
